...
static int load_vm_state(){
    int vmfd = open("/.cell",O_RDONLY);
	if(vmfd >= 0){
		char v[2] = {0};
		int ret = read(vmfd,v,1);
		if(ret > 0)
			property_set("persist.sys.exit", v);
		close(vmfd);
        return 1;
	}
    return 0;
...
static void load_persistent_properties() {
    ...
    while ((entry = readdir(dir.get())) != NULL) {
        ...
        if (strncmp("persist.sys.exit", entry->d_name, strlen("persist.sys.exit")) == 0) {
            if(load_vm_state())
                continue;
        }
...
void load_recovery_id_prop() {
    ...
    snprintf(fstab_filename, sizeof(fstab_filename), FSTAB_PREFIX "%s", "angler");
...
void load_system_props() {
    if(access("/.cell", F_OK)!= 0){
        ...
        INFO("load before props: %s\n", props_file_name);
        if(access(props_file_name, F_OK)== 0){
            load_properties_from_file(props_file_name, NULL);
            INFO("load props: %s\n", props_file_name);
        }else if(access("/system/build.VPDroid.prop", F_OK)== 0){
            load_properties_from_file("/system/build.VPDroid.prop", NULL);
            INFO("load props: /system/build.VPDroid.prop\n");
        }else{
            load_properties_from_file(PROP_PATH_SYSTEM_BUILD, NULL);
        }
    }
