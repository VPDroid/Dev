...
static int fuse_setup(struct fuse* fuse, gid_t gid, mode_t mask) {
    ...
    if(strcmp(value, "1") == 0)
    {
        ...
        property_get("ro.cell.sd.size", value, "25345104k");
        snprintf(opts, sizeof(opts),
        "fd=%i,rootmode=40000,default_permissions,allow_other,size=%s,user_id=%d,group_id=%d",
            fuse->fd,value,fuse->global->uid, fuse->global->gid);
    }else{*/
        snprintf(opts, sizeof(opts),
