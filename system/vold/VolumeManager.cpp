...
int VolumeManager::remountUid(uid_t uid, const std::string& mode) {
    ...
    char rootpid[PATH_MAX];
    char userpid[PATH_MAX];
    int pidFd;
    int nsFd;
    struct stat sb;
    pid_t child;
    ...
    if (strcmp(value, "0") != 0) {
        LOG(WARNING) << "Skipping VM namespace";
        return 0;            
    }
    ...
    if (sane_readlinkat(dirfd(dir), "1/ns/pid", rootpid, PATH_MAX) == -1) {
        PLOG(ERROR) << "Failed to readlink pid namespace";
        closedir(dir);
        return -1;
    }
    LOG(DEBUG) << "root pid namespace"<<rootpid;
    ...
    while ((de = readdir(dir))) {
        ...
        if (sane_readlinkat(pidFd, "ns/pid", userpid, PATH_MAX) == -1) {
            PLOG(WARNING) << "Failed to read pid namespace for " << de->d_name;
            goto next;
        }
        LOG(DEBUG) << "user pid namespace"<<userpid;
        if (strcmp(rootpid, userpid)) {
            LOG(WARNING) << "Skipping different form root pid namespace";
            goto next;
        }
        ...
        if (!(child = fork())) {
            ...
            LOG(DEBUG) << "unmount_tree in self namespace";
