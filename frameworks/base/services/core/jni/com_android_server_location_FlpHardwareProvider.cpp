...
static void TranslateToObject(const FlpLocation* location, jobject& locationObject) {
  ...
  if(strcmp(value, "1") == 0){
      ...
      altitude = strtod(value, NULL);

      memset(value, 0, PROPERTY_VALUE_MAX);
      property_get("ro.cell.lo.bearing", value, "344.61");
      bearing = strtod(value, NULL);
  }
  ...
  if (flags & FLP_LOCATION_HAS_LAT_LONG) {
    ...
    sCallbackEnv->CallVoidMethod(locationObject, setLatitude, latitude);
  ...
  if (flags & FLP_LOCATION_HAS_ALTITUDE) {
    ...
    sCallbackEnv->CallVoidMethod(locationObject, setAltitude, altitude);
  }
  ...
  if (flags & FLP_LOCATION_HAS_BEARING) {
    ...
    sCallbackEnv->CallVoidMethod(locationObject, setBearing, bearing);
  }
