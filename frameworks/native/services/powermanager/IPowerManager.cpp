...
public:
    ...
    virtual status_t wakeUp(long eventTime, const String16& reason, const String16& opPackageName)
    {
        Parcel data, reply;
        data.writeInterfaceToken(IPowerManager::getInterfaceDescriptor());
        data.writeInt64(eventTime);
    ...
    virtual status_t goToSleep(long eventTime, int reason, int flags)
    {
        Parcel data, reply;
        data.writeInterfaceToken(IPowerManager::getInterfaceDescriptor());
        data.writeInt64(eventTime);
        ...
        data.writeInt32(flags);
        // This FLAG_ONEWAY is in the .aidl, so there is no way to disable it
        return remote()->transact(GO_TO_SLEEP, data, &reply, IBinder::FLAG_ONEWAY);
    }
};
