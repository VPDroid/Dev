...
{
    return createSurfaceX(name, String8(""), w, h, format, flags, handle, gbp);
}

status_t Client::createSurfaceX(
        const String8& name, const String8& systemname,
        uint32_t w, uint32_t h, PixelFormat format, uint32_t flags,
        sp<IBinder>* handle,
        sp<IGraphicBufferProducer>* gbp)
{
    ...
    public:
        MessageCreateLayer(SurfaceFlinger* flinger,
                const String8& name, const String8& systemname, Client* client,
                uint32_t w, uint32_t h, PixelFormat format, uint32_t flags,
                sp<IBinder>* handle,
                sp<IGraphicBufferProducer>* gbp)
            : flinger(flinger), client(client),
              handle(handle), gbp(gbp),
              name(name), systemname(systemname), w(w), h(h), format(format), flags(flags) {
        }
        status_t getResult() const { return result; }
        virtual bool handler() {
            result = flinger->createLayer(name, systemname, client, w, h, format, flags,
                    handle, gbp);
            return true;
        }
    };
    ...
    sp<MessageBase> msg = new MessageCreateLayer(mFlinger.get(),
            name, systemname, this, w, h, format, flags, handle, gbp);
