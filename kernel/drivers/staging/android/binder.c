...
static struct binder_node *binder_context_mgr_nodes[MAX_CONTEXT] = {0};
static struct binder_node *get_binder_ns_context_mgr_node(int i)
{
	if(i >= 0 && i < MAX_CONTEXT)
		return binder_context_mgr_nodes[i];
	return NULL;
};

static int current_drv_ns_cell(void)
{
	char tag[DRV_NS_TAG_LEN];
	int index = 0;

	if(current_drv_ns() == &init_drv_ns){
		pr_err("current_drv_ns_cell  index = %d\n", index);
		return index;
}

	for(index = 1; index < MAX_CONTEXT; index++){
		sprintf(tag, "cell%d", index);
...
static int mgr_node(struct binder_node *node)
{
	...
	for(i = 0; i < MAX_CONTEXT; i++){
		if(node == binder_context_mgr_nodes[i]){
			return 1;
		}
	}
						  ...
						  struct binder_node *node)
{
	...
	{
		if(node == binder_context_mgr_nodes[i]){
			new_ref->desc = INIT_CONTEXT_MGR_HANDLE + i;
			break;
		}
	}
	if(node == proc->binder_ns->context_mgr_node)
		new_ref->desc = 0;

	if(new_ref->desc < INIT_CONTEXT_MGR_HANDLE){
		for (n = rb_first(&proc->refs_by_desc); n != NULL; n = rb_next(n)) {
			ref = rb_entry(n, struct binder_ref, rb_node_desc);
			if(ref->desc >= INIT_CONTEXT_MGR_HANDLE)
				continue;
			       ...
			       struct binder_transaction_data *tr, int reply)
{
	...
	if (reply) {
		...
		if(tr->target.handle == 0){
			...
			target_node = get_binder_ns_context_mgr_node(tr->target.handle - INIT_CONTEXT_MGR_HANDLE);
			if (target_node == NULL) {
				return_error = BR_DEAD_REPLY;
				goto err_no_context_mgr_node;
			}
			...
			binder_size_t *consumed)
{
	...
	while (ptr < end && thread->return_error == BR_OK) {
		...
		case BC_DECREFS: {
			...
			}else if (target >= INIT_CONTEXT_MGR_HANDLE && get_binder_ns_context_mgr_node(target - INIT_CONTEXT_MGR_HANDLE) &&
			    (cmd == BC_INCREFS || cmd == BC_ACQUIRE)) {
				ref = binder_get_ref_for_node(proc,
				       get_binder_ns_context_mgr_node(target - INIT_CONTEXT_MGR_HANDLE));
				if (ref->desc != target) {
					binder_user_error("%d:%d tried to acquire reference to desc 1023, got %d instead\n",
						proc->pid, thread->pid,
						ref->desc);
				}
			} 
...
retry:
	...
	while (1) {
		...
		if (t->from) {
			...
			if(get_drv_ns_by_task(sender) != &init_drv_ns)
				tr.sender_systemmode = 1;
			else
				tr.sender_systemmode = 0;
		} else {
			tr.sender_pid = 0;
...
static long binder_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case BINDER_SET_CONTEXT_MGR:
		...
		index = current_drv_ns_cell();
		if(index >= 0){
			binder_context_mgr_nodes[index] = proc->binder_ns->context_mgr_node;
		}
		break;
...
static int binder_node_release(struct binder_node *node, int refs)
{
	...
	dead_nodes = &node->proc->binder_ns->dead_nodes;
	node->proc = NULL;
	node->local_strong_refs = 0;
	node->local_weak_refs = 0;
	hlist_add_head(&node->dead_node, dead_nodes);
...
static void binder_deferred_release(struct binder_proc *proc)
{
	...
	if (proc->binder_ns->context_mgr_node &&
		...
		for(i = 0; i < MAX_CONTEXT; i++){
			if(binder_context_mgr_nodes[i] == proc->binder_ns->context_mgr_node){
				binder_context_mgr_nodes[i] = NULL;
				break;
			}
		}
...
static void __binder_state_show(struct drv_ns_info *drv_ns_info, void *data)
{
	struct binder_drv_ns *binder_ns;
...
#ifdef CONFIG_DRV_NS
	loop_drv_ns_info(binder_ns_id, m, __binder_state_show);
#else
	__binder_state_show(&init_binder_ns.drv_ns_info, m);
#endif
...
static void __binder_stats_show(struct drv_ns_info *drv_ns_info, void *data)
{
	struct binder_drv_ns *binder_ns;
	struct seq_file *m = data;
	struct binder_proc *proc;
...
#ifdef CONFIG_DRV_NS
	loop_drv_ns_info(binder_ns_id, m, __binder_stats_show);
#else
	__binder_stats_show(&init_binder_ns.drv_ns_info, m);
#endif
...
static void __binder_transaction_show(struct drv_ns_info *drv_ns_info,
				      void *data)
{
	struct binder_drv_ns *binder_ns;
	struct seq_file *m = data;
...
#ifdef CONFIG_DRV_NS
	loop_drv_ns_info(binder_ns_id, m, __binder_transaction_show);
#else
	__binder_transaction_show(&init_binder_ns.drv_ns_info, m);
#endif
...
static void __binder_proc_show(struct drv_ns_info *drv_ns_info,
				      void *data)
{
	struct binder_drv_ns *binder_ns;
	struct seq_file *m = data;
...
#ifdef CONFIG_DRV_NS
	loop_drv_ns_info(binder_ns_id, m, __binder_proc_show);
#else
	__binder_proc_show(&init_binder_ns.drv_ns_info, m);
#endif
